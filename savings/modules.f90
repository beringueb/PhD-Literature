    ...
    character(LEN=*), parameter :: version = 'Nov16_rayleigh'

    integer, parameter  :: num_cmb_freq =  5 ! 2017 Benjamin B 
    logical :: rayleigh_diff  ! 2017 Benjamin B Rayleigh_diff should be read from params.ini file
    logical :: rayleigh_pows(3) = [.true.,.true.,.true.]
    logical :: rayleigh_back_approx = .true.  !changed !!!!!!!!!!!
...
500     if (L< lmax_extrap_highl) &
    ...
    real(dl) tb(nthermo),cs2(nthermo),xe(nthermo),xrayleigh(nthermo)
    ...
    ! 2017 Benjamin Beringue ; removed thomson part 
    if (rayleigh_back_approx) then
        a2=a**2
        total_scattering_eff= Recombination_xe(a) + Recombination_rayleigh_eff(a)*(min(1._dl,& !  Removed
    ...
    external rombint
    real(dl) dq, q, dlfdlq
    logical :: plot_scatter = .false. !!! changed 2017 Benjamin B
    real(dl) elec_fac
    real(dl), parameter :: nu_eff = 3101692._dl !3125349._dl is approx from Yu paper

    if (num_cmb_freq<30) then
          phot_freqs(1:5) = [0,93,145,225,280]   !  2017 Benjamin B changed to SO freqs 
    ...
    do i=2,nthermo
       ...
       ! 2017 Benjamin B ; switched off rayleigh effect (undone) and added xrayleigh to ouput x_rayleigh_eff
        ...
        xrayleigh(i) = Recombination_rayleigh_eff(a)
        ...
        !! 2017 Benjamin Beringue

        !print * ,i,dotmu(i,1),sdotmu(i,1);
    ...
    !! 2017 Benjamin B, modified the output of the file

    if (plot_scatter) then
        call CreateTxtFile('/home/bb510/Code/CAMB/outputs/visibilities.txt',1)
        do j1=1,nthermo !!!!
            tau = tauminn*exp((j1-1)*dlntau)
            write(1,'(9ES15.5E3)') tau, scaleFactor(j1), dotmu(j1,1)*scaleFactor(j1)**2/akthom, &
                 real(emmu(j1,1)*dotmu(j1,1)),real(emmu(j1,3:7)*emmu(j1,1)*dotmu(j1,3:7)) !vis = taudot*exp(-tau_tot) !! 
        end do
        close(1)
        
        call CreateTxtFile('/home/bb510/Code/CAMB/outputs/taudot.txt',1)
        do j1=1,nthermo !!!!
            tau = tauminn*exp((j1-1)*dlntau)
            write(1,'(9ES15.5E3)') tau, scaleFactor(j1), dotmu(j1,4)*scaleFactor(j1)**2/akthom, &
                 real(dotmu(j1,1)),real(dotmu(j1,3:7))
        end do
        close(1)
        
        call CreateTxtFile('/home/bb510/Code/CAMB/outputs/out_xe_tau.txt',1)
        do j1=1,nthermo !!!!
            tau = tauminn*exp((j1-1)*dlntau)
            write(1,'(9ES15.5E3)') tau, xe(j1)
        end do
        close(1)

        call CreateTxtFile('/home/bb510/Code/CAMB/outputs/out_ray_tau.txt',1)
        do j1=1,nthermo !!!!
            tau = tauminn*exp((j1-1)*dlntau)
            write(1,'(9ES15.5E3)') tau, xrayleigh(j1)
        end do
        close(1)
        stop
    end if

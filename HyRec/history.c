...
Added Januray 2018 : Modified the function to output xrayleigh_output and xHe_output
****************************************************************************************************/
...
void rec_build_history(REC_COSMOPARAMS *param, HRATEEFF *rate_table, TWO_PHOTON_PARAMS *twog_params,
   ...
   for(iz=0; iz<param->nz && Delta_xe > 1e-8; iz++) {
      ...
      xrayleigh_output[iz] = 1.-xe_output[iz];
   }
   ...
   for(; iz < param->izH0+1; iz++) {
        ...
        xrayleigh_output[iz] = xH1s+ 0.1*param->fHe*(1.-xHeII);

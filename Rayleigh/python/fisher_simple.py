...
def def_cov_matrix(data, do_pol, do_rayleigh, freqs) :  
    ...
    if do_rayleigh :
        ...
        if do_pol :
            ...
            cov[:,0,1] = data[:,0,0,2]
            cov[:,1,0] = data[:,0,0,2]
            cov[:,1,1] = data[:,0,0,1] + noise.values[l_min-2:l_max-1,1+len(freq_tot)] 
        else :
...
freqs = [0,93,145,225,280,350,405,862]
experiment = 'CCAT-SO'
param_file_root = "/home/bb510/Code/CAMB/"
noise_file = '/home/bb510/Code/Rayleigh/noise/noise_CCAT_SO_PLANCK.txt'
l_min = 2
l_max = 3000 
...
for do_pol in [True] :
    for do_rayleigh in [True,False] :
        ...
        if do_rayleigh :
           ...
           str2 = 'nor'
        np.savetxt('../fisher_matrices/fisher_{}_{}_CCAT_SO_PLANCK_full_params_DM.txt'.format(str2,str1), fisher, delimiter = '   ', newline = '\n')
        update = False
print("Done !")

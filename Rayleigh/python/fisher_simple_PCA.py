...
def run_camb(position) :
    subprocess.call("./camb {}".format(param_file_root + "PCA_init/params_PCA_{:07.2f}.ini".format(position)), shell = True, cwd = "/home/bb510/Code/CAMB/", stdout = subprocess.DEVNULL)


def modify_param_PCA(experiment, position, width) :
    ...
    with open(param_file_init) as f:
        ...
        for line in f :
            ...
            if "output_root" in line and not('#' in line) :
                line = "output_root  = /store/DAMTP/bb510/Rayleigh/Data/camb_out/PCA/{}_{:07.2f} \n".format(experiment,position)
            newText += line 
    out_file = param_file_root + "PCA_init/params_PCA_{:07.2f}.ini".format(position) 
    with open(out_file, "w") as f:
        f.write(newText)


def reading_data(experiment, position, freqs, freq_tot) :
    data_file_root =  "/store/DAMTP/bb510/Rayleigh/Data/camb_out/PCA/"
    data = np.zeros((l_max-l_min+1, len(freqs), len(freqs), 3))
    for i in range(len(freqs)) : 
        for j in range(len(freqs)) :
            data_read = pd.read_csv(data_file_root+'{}_{:07.2f}__{:d}_{:d}'.format(experiment,position, freq_tot.index(freqs[i])+1, freq_tot.index(freqs[j])+1), header = None, skiprows = 0, sep = '\s+' )

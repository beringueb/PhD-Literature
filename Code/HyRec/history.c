...
/*                             - Implemented Principal componenent analysis                           */
/******************************************************************************************************/ 
...
int firstTime = 0;
double *xe_output, *xrayleigh_output, *delta_u_output, *tm_output, *ts_output;
RAD_OUTPUTS rad; 
double logstart;
...
void rec_build_history_camb_(const double* OmegaC, const double* OmegaB, const double* OmegaN, const double* Omegav 
         ...
         , const double* fsR, const double* meR, const double* mean_x_PCA, const double* width_x_PCA, const double* amplitude_sign, const bool* do_PCA, const int* feedback) {
  ...
  param.amplitude_sign = *amplitude_sign; /*sign of the perurbation (+/-1, 2018)*/
  ...
  if(*feedback > -1) { 
  	 ...
  	 fprintf(stderr, " alpha/alpha_0 = %e \n", param.fsR);
         fprintf(stderr, " position of gaussian bump %e \n", param.mean_x);
         fprintf(stderr, " width of gaussian bump %e \n", param.width_x); 
         fprintf(stderr, " sign of the perturbation %e \n", param.amplitude_sign); 
  	 fprintf(stderr, " ********HYREC******************** \n");
  }
  ...
  sprintf(file_name, "/home/bb510/Code/Rayleigh/visibilities/PCA/xe_pos_%07.2f_%1.0f.txt",param.mean_x, param.amplitude_sign);
  ...
  if (*do_PCA){
     ...
     for (izz=0; izz<param.nz; izz++) {
         ...
         xe_output[izz] = xe_output[izz] + param.amplitude_sign*xe_output[izz]*delta_u_output[izz]*0.01;  /*if do_PCA, then actually modify the ionization fraction, including the gaussian bump (added Januray 2018)*/
         fprintf(fout_xe,"%f      %f \n",z,xe_output[izz]);
...
Added Januray 2018 : - Modified the function to output xrayleigh_output 
                     - Modified the function to output delta_u_output, systematic perturbation of xe in a PCA
****************************************************************************************************/
...
void rec_build_history(REC_COSMOPARAMS *param, HRATEEFF *rate_table, TWO_PHOTON_PARAMS *twog_params,
   ...
   for(iz=0; iz<param->nz && Delta_xe > 1e-8; iz++) {
      ...
      delta_u_output[iz] = exp(-(z-param->mean_x)*(z-param->mean_x)/(2*param->width_x*param->width_x));
   }

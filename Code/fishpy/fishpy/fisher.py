...
def plot_triangular(setup, list_fisher, parameter_list=None):
    ...
    label_list = ['CCAT-p TT+TE+EE+Rayleigh','CCAT-p TT+TE+EE','PLANCK TT+TE+EE']
    ...
    for i in range(n_params):
        for j in range(i+1):
            if i != j:
                ...
                for label in axs[i,j].xaxis.get_ticklabels()[::2]:
                    label.set_visible(False)
            axs[i,j].tick_params('both', labelsize = 13)
            axs[0,0].set_yticklabels([])
            axs[n_params-1,j].set_xlabel(names[parameter_list[j]], fontsize = 18)
            ...
            axs[0,0].legend(loc = 'center',prop = {'size':16},fancybox = True, shadow = True,bbox_to_anchor = (1.5,0.45))
            if i != n_params-1 :
                axs[i,j].set_xticklabels([])
            if j != 0 :
                axs[i,j].set_yticklabels([])
            if i != 0:
                axs[i,0].set_ylabel(names[parameter_list[i]], fontsize = 18)

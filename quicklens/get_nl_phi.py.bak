...
def include_delensing_noise(experiment) : 
    ...
    file_name = "../Rayleigh/noise/noise_%s.txt" % experiment
    print(file_name)
    try : 
        noise_in = np.loadtxt(file_name)
    except : 
        print("File doesn't exist, need to generate it first !")
    lmax = int(noise_in[-1,0])
    ...
    nltt = np.concatenate([np.zeros(2), noise_in[0:lmax-1,1]*2*np.pi/l/(l+1.)])
    nlee = nlbb = np.concatenate([np.zeros(2), noise_in[0:lmax-1,1+n_freq]*2*np.pi/l/(l+1.)])
    ...
    print(np.shape(l))
    print(np.shape(noise_tot[2:lmax+1]))
    print(np.shape(noise_in))
    
    noise_write = np.concatenate([np.reshape(l,(lmax-1,1)),noise_in[:,1:],np.reshape(noise_tot[2:lmax+1],(lmax-1,1))], axis = 1)
    print(np.shape(noise_write))
    np.savetxt(file_name.replace('.txt','_lens.txt'), noise_write)
...
if __name__ == "__main__" :
    ...
    nlpp_TT_fullsky, nlpp_EB_fullsky = include_delensing_noise('PLANCK') 
